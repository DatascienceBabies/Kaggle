...
class Classifications:
    ...
    def add_logistic_regression(self, cross_validation=5, rnd_state=0, max_iter=500):
    ...
    def randomized_random_forest_parameter_search(self, train_X, train_Y):
        ...
        rf_random = RandomizedSearchCV(
            estimator=rf, param_distributions=param_grid, n_iter=100, cv=3, verbose=2, random_state=42, n_jobs=-1)
    ...
    def add_random_forest(self, max_features='auto', max_depth=5, n_estimators=5000, min_samples_split=4, min_samples_leaf=1, bootstrap=True):
        rfc = RandomForestClassifier(max_features=max_features, max_depth=max_depth, n_estimators=n_estimators,
                                     min_samples_split=min_samples_split, min_samples_leaf=min_samples_leaf, bootstrap=bootstrap)
        self.algorithms['Random Forest'] = rfc

    def add_k_neighbors(self, n_neighbors=5):
        neigh = KNeighborsClassifier(n_neighbors=n_neighbors)
        self.algorithms["K Neighbors"] = neigh

    def add_gaussian_proccess(self):
        self.algorithms["Gaussian Process"] = GaussianProcessClassifier(1.0 * RBF(1.0))

    def add_mlp(self, alpha=1, max_iter=1000):
        self.algorithms["MLP"]=MLPClassifier(alpha=alpha, max_iter=max_iter)

    def add_ada_boost(self):
        ada_boost=AdaBoostClassifier()
        self.algorithms["ADA Boost"]=ada_boost

    def add_quadratic_discriminant_analysis(self):
        qda=QuadraticDiscriminantAnalysis();
        self.algorithms["Quadratic Discriminant Analaysis"]=qda
    ...
    def find_best_algorithm(self, train_x, train_y, test_x, test_y):
        ...
        print('Best Model is: {0} with a score of {1} - will continue with this'.format(
